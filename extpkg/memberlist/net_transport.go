...
type NetTransport struct {
	config       *NetTransportConfig // 配置
	packetCh     chan *Packet        // 已接收的upd数据包
	streamCh     chan net.Conn       // 已接收的tpc conn
	logger       *log.Logger
	wg           sync.WaitGroup
	tcpListeners []*net.TCPListener // tcp连接
	udpListeners []*net.UDPConn     // udp连接
	shutdown     int32              // 是否已关闭
}
...
func NewNetTransport(config *NetTransportConfig) (*NetTransport, error) {
	...
	for _, addr := range config.BindAddrs {
		...
		// else.
		// 如果端口字段为0，函数将选择一个当前可用的端口，可以用Listener的Addr方法获得该端口
		if port == 0 {
...
func (t *NetTransport) FinalAdvertiseAddr(ip string, port int) (net.IP, int, error) {
	...
	if ip != "" { // 已配置advertiseAddr
		...
		// 如果bind地址是"0,0,0,0"，那么advertiseAddr从网络中选择一个ip，否则就直接使用bind地址
